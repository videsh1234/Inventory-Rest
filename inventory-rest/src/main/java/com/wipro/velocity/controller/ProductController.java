...
public class ProductController {
	...
	public ResponseEntity<Product> updateProduct(@PathVariable(value="id") String pId,@Validated @RequestBody Product p) throws 
	ResourceNotFoundException
	{
		Product product=prepo.findById(pId).orElseThrow(()->new ResourceNotFoundException("product not found for this id: "+pId));
	...
	public Map<String,Boolean> deleteProduct(@PathVariable(value="id") String pId) throws 
	ResourceNotFoundException
	{
		Product product=prepo.findById(pId).orElseThrow(()->new ResourceNotFoundException("product not found for this id: "+pId));
		
		prepo.delete(product); //delete() method is predefined in MongoRepository
		
		Map<String,Boolean> response=new HashMap<>();
		response.put("Delete the product", Boolean.TRUE);
		return response;
	}
	
	// GET - http://localhost:9095/ims/api/products/101
	@GetMapping("/products/{id}")
	public ResponseEntity<Product> getProductById(@PathVariable(value="id") String pId)throws ResourceNotFoundException
	{
		Product product=prepo.findById(pId).orElseThrow(()->new ResourceNotFoundException("product not found for this id: "+pId));
	...
	{
		return (List<Product>) prepo.findByMadein(madein); //invokes  findByMadein custom method of MongoRepository
